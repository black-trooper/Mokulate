...
- Mokulate 左右本体
- GitHub Releases からダウンロードしたファームウェア  
  - `Mokulate_left.uf2`  
  - `Mokulate_right.uf2`
...
2. 基板上の XIAO の USB Type-C 端子を PC に接続する。


### ブートローダーモードへ移行

1. XIAO 基板上の **リセットボタン（BOOT / RST）** を確認する。
2. **リセットボタンを素早く 2 回押す（ダブルクリック）**。
3. PC に **「XIAO SENSE」という名前の仮想 USB ドライブ** が表示される。
